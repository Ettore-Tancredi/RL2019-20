...
long long int check()
{
  ...
  {
    if(colors[checkingSet[i].r][checkingSet[i].g][checkingSet[i].b] == checkingSet[i].C)
      ++validi;
  }

  return ((validi * 10000 ) / check_dim);
}
...
int main()
{
  ...
  string inFileName = "allenamento.txt";
  ...
  inFileName = "check.txt";
  ifstream check_file(inFileName);

  while(!check_file.eof())
  {
    check_file >> tempPx.r >> tempPx.g >> tempPx.b >> color;
